...
RSpec.describe(AnalyticsService, type: :service) do
  ...
  let(:entry1) { LogEntryModel.new(path: '/index/1', ip: '127.0.0.1', line_number: 1) }
  let(:entry2) { LogEntryModel.new(path: '/index/1', ip: '127.0.0.1', line_number: 2) }
  let(:entry3) { LogEntryModel.new(path: '/index', ip: '127.0.0.1', line_number: 3)   }
  let(:entry4) { LogEntryModel.new(path: '/index', ip: nil, line_number: 4)           }
  let(:entry5) { LogEntryModel.new(path: nil, ip: '127.0.0.1', line_number: 5)        }
  let(:entry6) { LogEntryModel.new(path: '/index/1', ip: '127.0.0.2', line_number: 6) }
  let(:entry7) { LogEntryModel.new(path: '/index/1', ip: '127.0.0.3', line_number: 7) }
  ...
  describe '#errors' do
    subject(:errors) { analytics_service.errors }
